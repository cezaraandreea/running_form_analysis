...
class RunningAnalyzer:
    ...
    def _generate_summary(self, issues: list[FeedbackItem], score: float) -> str:
        ...
        if score >= 85:
            verdict = "✅ **Tehnică excelentă!** Parametrii biomecanici sunt în limite optime."
        elif score >= 65:
            verdict = "⚠️ **Tehnică bună**, cu câteva aspecte ce pot fi îmbunătățite."
        else:
            verdict = "❌ **Sunt detectate probleme semnificative** care îți afectează eficiența și cresc riscul de accidentare."
        ...
        parts = []
        if errors:
            parts.append(f"🔴 {_n(len(errors), 'problemă critică', 'probleme critice')}")
        if warnings:
            parts.append(f"🟡 {_n(len(warnings), 'avertizare', 'avertizări')}")
        if oks:
            parts.append(f"🟢 {_n(len(oks), 'aspect pozitiv', 'aspecte pozitive')}")

        counts = " · ".join(parts) if parts else "Nicio problemă detectată."
        return f"{verdict}\n\n{counts}"